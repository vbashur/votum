...
public class PlayerComponent extends CustomComponent {
	...
	private void buildPits() {
		...
		if (player == Player.SPOUNGE_BOB) {
			this.buildPlayer1Pits(nameLabel);

		} else {
			this.buildPlayer2Pits(nameLabel);
		}
	}

	private void buildPlayer1Pits(Label nameLabel) {
		...
		for (int iter = 0; iter <= Const.DEFAULT_PIT_NUM; ++iter) {
			...
			if (iter == 0) {
				...
				b.addClickListener(new PitClickListener(buttonIndex, player, arbiter));
	...
	private void buildPlayer2Pits(Label nameLabel) {
	...
	public static class PitClickListener implements ClickListener {
		...
		public PitClickListener(int buttonIndex, Player player, GravaArbiter arbiter) {
			super();
